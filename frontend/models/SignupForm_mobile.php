...
class SignupForm extends Model
{
    ...
    {
        return [
            [['username','ncname','mobile','password','repassword'], 'filter', 'filter' => 'trim'],
            [['username','ncname','mobile','password','repassword'], 'string', 'min' => 2, 'max' => 255],
    ...
    public function signup()
    {
        if ($this->validate()) {
            ...
            $this->mobile = HtmlPurifier::process($this->mobile, ['HTML.Allowed' => '']);
            $this->ncname = HtmlPurifier::process($this->ncname, ['HTML.Allowed' => '']);
            $user = new User();
            $user->scenario="register_frontend";
            $user->username = $this->mobile;
            $user->mobile = $this->mobile;
            $user->email = $this->mobile . '@meiling.cn';
