$(function () {
    function stretch_chatvideo() {
        ...
        var leftarea = chatvideo.find(".right-area");
        var handler = chatvideo.find(".left-area .handler");
        var rightarea = chatvideo.find(".left-area");
        ...
        chatvideo.find(".chat-area").resize(function () {
            var isadmin = room_info.isadmin;
            var stretch_if = false;
            var width = $(window).width();
            var chat_width = $(this).width();
            if (width > 768 && isadmin && chat_width < 720) {
                stretch_if = true;
            }
            else if (width < 768 && isadmin) {
                stretch_if = true;
            }
        ...
        chatvideo.find(".chat-area").resize();
        /*拉伸chat-video*/
    }
        ...
        if (Live_id && Live_id == '-1') {
            layer.open({
                ...
                success: function (layero, index) {
                    ...
                    $.cookie('guest_unable_video', 1, {expires: 30, path: '/'});
                }
            });
        }
        else {
    ...
    function iframe_layer(url, width, height,skin) {
        skin = skin ? skin : "";
                    ...
                    if (!result.error) {
                        ...
                        if (popwindows) {
                            ...
                            for (var i in popwindows) {
                                ...
                                var area = [];
                                if (popwindow.pwidth > 0 || popwindow.pheight > 0) {
                                    area = [parseInt(popwindow.pwidth) + "px", parseInt(popwindow.pheight) + "px"];
                                }
                                else if (popwindow.pwidth > 0) {
                                    area = [parseInt(popwindow.pwidth) + "px"];
                                }
                                else {
                                    area = ['800px'];
                                }
    ...
    function Userlist() {
        ...
        this.adduser = function (lineinfo, $add) {
            ...
            $html = $.format($html, lineinfo.id, lineinfo.uid, lineinfo.fd, lineinfo.name, lineinfo.ip, lineinfo.rid, lineinfo.rolename, lineinfo.img ? lineinfo.img : lineinfo.roleimg, lineinfo.type);
        ...
        this.init = function () {
            self.initevent();
            self.loadUMore('member', 1, 20, function () {
                self.loadUMore('admin', 1, 20);
            });
            return this;
        };
        /*userlist*/
    ...
    function Auth() {
        ...
        this.initevent = function () {
            ...
            self.loginarea.find(".tuiguanglink").click(function(){
                ...
                layer.open({
                    ...
                    title: "独立推广链接",
                    area:['500px'],
                    content: "<div style='font-size:15px;line-height: 2em;text-align: center;color: #333;'>"+href+"</div>"
                });
            });

            self.loginarea.find(".loginbtn").click(function () {
                self.iframelayer = layer.open({
                    title: false,
                    type: 2,
                    area: ['500px','375px'],
                    shadeClose: true, /*点击遮罩关闭*/
                    content: "/site/load_iframe?name=login",
                    success: function () {
                    }
                });
                /*点击登录*/
            ...
            self.loginarea.find(".regbtn").click(function () {
                self.iframelayer = layer.open({
                    ...
                    area: ['500px','445px'],
                    shadeClose: true, /*点击遮罩关闭*/
                    content: "/site/load_iframe?name=reg",
                    success: function () {
                    }
                });
                /*点击注册*/
            ...
            self.loginarea.find(".changepass").click(function () {
                self.iframelayer = layer.open({
                    ...
                    area: ['547px','332px'],
                    shadeClose: true, /*点击遮罩关闭*/
                    content: "/site/load_iframe?name=resetpass",
                    success: function () {
                    }
                });
                /*点击修改密码*/
            ...
            self.loginarea.find(".changenick").click(function () {
                ...
                self.iframelayer = layer.open({
                    ...
                    area: ['547px','220px'],
                    shadeClose: true, /*点击遮罩关闭*/
                    content: "/site/load_iframe?name=resetnick",
                    success: function () {
                    }
                });
            ...
            self.loginarea.find(".changephoto").click(function () {
                self.iframelayer = layer.open({
                    title: false,
                    type: 2,
                    area: ['547px','350px'],
                    shadeClose: true, /*点击遮罩关闭*/
                    content: "/site/load_iframe?name=changephoto",
                    success: function () {
                    }
                });
                /*点击修改头像*/
            ...
            self.loginarea.find(".zhibocontrol").click(function () {
                self.iframelayer = layer.open({
                    title: false,
                    type: 2,
                    area: ['547px','550px'],
                    shadeClose: true, /*点击遮罩关闭*/
                    content: "/site/load_iframe?name=zhibocontrol",
                    success: function () {
                    }
                });
                /*点击添加子账号*/
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (!result.error && result.data) {
                        ...
                        room_info.userinfo.img = result.data.img;
                        ...
                        self.userinfoarea.find(".rolepic").attr("src", room_info.userinfo.img ? room_info.userinfo.img : room_info.userinfo.roomrolepic);
    ...
    function Chat() {
        ...
        this.likezhibo_area = $("#dianzan");
        this.likezhibo_click = true;
        this.Dianzan = new DianZaner();

        /**账户切换**/
        this.select_account_button = this.chat_handle_area.find("#select_account");
        /**飞屏模式**/
        this.fly_mod_button = this.chat_handle_area.find("#fly_mod");
        ...
        this.chat_history_page = 0;
        /*当前加载聊天历史的页数*/
        this.chat_history_pagenum = 10;
        /*每次加载聊天历史的条数*/
        ...
        /*是否开启滚动*/
        this.caitiao_lefttime = 0;
        /*彩条发送限制剩余时间*/
        this.publish_chat_lefttime = 0;
        /*公聊限制剩余时间*/
        ...
        this.initevent = function () {
            ...
            self.message_panel.load(function () {
            ...
            self.sendbutton.click(function () {
                ...
                self.sendCommonmessage(self.fly_mod_button.is(':checked') ? 'fly' : 'text', text, "public");
            ...
            /***喊单提醒***/
            $("body").delegate(".handan_detail .viewbilldetail", "click", function () {
                iframe_layer("/site/show-jian-cang", 800, 600);
            });

            $("body").delegate(".pingcang_detail .viewbilldetail", "click", function () {
                iframe_layer("/site/show-ping-cang", 800, 600);
            });

            /***垂直公告***/
            $('#marquee2').kxbdSuperMarquee({isEqual: false, distance: 40, time: 5, direction: 'up'});

            /***直播点赞***/

            self.likezhibo_area.find(".heart").on("click", function () {

                var likeNum = parseInt(self.likezhibo_area.find(".like-num").html());

                if (self.likezhibo_click == true) {
                    self.likezhibo_click = false;
                    likeNum += 1;
                    if ($(".icon-aixin")) {
                        self.Dianzan.removeIcon();
                    }
                    self.Dianzan.createDianZaner(this, 2);
                    self.likezhibo_area.find(".like-num").html(likeNum);
        ...
        this.make_other_accounts = function (accounts) {
            if (this.select_account_button.length > 0 && accounts) {
                for (var i in accounts) {
                    var item = accounts[i];
                    var account_option_str = "<option value='{0}' from='{0}' fromname='{1}' from_roleid='{2}' from_pic='{3}'>{1}</option>";
                    var uitem_str = $.format(account_option_str, item.from, item.fromname, item.from_roleid, item.from_pic);
        ...
        this.initmaterial_later = function () {
            ...
            $("body").delegate("#deskback_area img", "click", function () {
                ...
                $.cookie('deskback_name', name, {expires: 1800, path: '/'});
            });
            ...
            if (deskback_name) {
                $("body").css("background", "url(" + room_info.deskbacks['list'][deskback_name] + ")");
            }
        ...
        this.showNewMsg = function (dataObj, returned) {
            ...
            message.from_pic = dataObj.from_pic ? dataObj.from_pic : message.from_rolepic;
            message.to_pic = dataObj.to_pic ? dataObj.to_pic : message.to_rolepic;
            ...
            if (!dataObj.type || dataObj.type == 'text' || dataObj.type == 'fly') {/*没有类型或者类型为text的话*/
                ...
                message.content = dataObj.data;
            }

            if(room_info.chat_forbidden_words_config && Array.isArray(room_info.chat_forbidden_words_config) && room_info.chat_forbidden_words_config.length>1){
                message.content = message.content.replace(new RegExp(room_info.chat_forbidden_words_config.join('|'),'img'),'*');/*违禁词替换*/
            }
            message.content = parseXss(message.content);
            /*替换表情*/
            message.time = dataObj.time ? dataObj.time : GetDateT();
            message.channal = dataObj.channal;
            /*公聊还是私聊*/
            ...
            if ($.inArray(message.channal, [0, 1]) != -1) {
                ...
                if (!message.to_uid && !message.to_name) {
                    msgitem_str = "<li class='msgitem " + dataObj.type + (message.ischeck ? " " : " nocheck") + " clearfix' msgid='{0}' from_uid='{1}' from_name='{2}' from_roleid='{3}'><span class='time'>{4}</span><img class='rolepic' src='{5}'><span class='nickname from'>{7}</span> <div class='msg'><span class='arrow'></span>{6}</div><div class='handle_area'>{8}<!--操作区域--></div></li>";
                    msgitem_str = $.format(msgitem_str, message.msgid, message.from_uid, message.from_name, message.from_roleid, message.time, message.from_pic, message.content, message.fromname_display, handle_str);
                }
                else {
                    msgitem_str = "<li  class='msgitem " + dataObj.type + (message.ischeck ? " " : " nocheck") + " clearfix' msgid='{0}' from_uid='{1}' from_name='{2}' from_roleid='{3}' to_uid='{4}' to_name='{5}' to_roleid='{6}'><span class='time'>{7}</span><img class='rolepic' src='{8}'><span class='nickname from'>{10}</span><div class='totip'>对<span class='toarrow'></span><!--totip--></div><span class='nickname to'>{11}</span><div class='msg'><span class='arrow'></span>{9}</div><div class='handle_area'>{12}<!--操作区域--></div></li>";
                    msgitem_str = $.format(msgitem_str, message.msgid, message.from_uid, message.from_name, message.from_roleid, message.to_uid, message.to_name, message.to_roleid, message.time, message.from_pic, message.content, message.fromname_display, message.toname_display, handle_str);
                }

                if (message.color) {
                    var $msgitem = $(msgitem_str);
                    $msgitem.find(".msg").addClass("color_one").css({"color": message.color});
                ...
                if (!returned) {
                    if (message.type == "fly") {
                        self.showFlymsg($(msgitem_str).find(".msg").get(0).outerHTML);
                        /**显示飞屏**/
                    }
                ...
                msgitem_str = $.format(msgitem_str, message.msgid, message.from_uid, message.from_name, message.from_roleid, message.time, message.from_pic, message.content, message.fromname_display);
                if (!returned) {
                    var $private_ul;
                    var private_ui;
                    var selfuid = room_info.userinfo.uid;
        ...
        this.makemsg = function (type, data, chattype) {
            ...
            if (select_account_option.length > 0) {
                ...
                msg.from_pic = (select_account_option.attr("from_pic") != "null") ? select_account_option.attr("from_pic") : "";
                if(!msg.from_pic && select_account_option.attr("from_roleid")){
                    msg.from_pic = GetRoleAttr(select_account_option.attr("from_roleid"), 'role_pic');
                }
                ...
                msg.from_pic = room_info.userinfo.img;
            ...
            if (chattype == "public") {
            ...
            else if (chattype == "systip") {
                msg.from = 0;
                msg.fromname = "";
                msg.from_roleid = room_info.guestroleid;
                msg.to = 0;
                msg.toname = "";
                msg.to_roleid = 0;
                msg.channal = 0;
                ...
                msg.fromname = "系统消息";
                msg.from_roleid = 0;
                msg.from_pic = GetRoleAttr(room_info.guestroleid, 'role_pic');
                msg.to = 0;
                msg.toname = "";
                msg.to_roleid = 0;
                msg.channal = 0;
        ...
        this.showSysmsg = function (type, content) {
            if (!type)type = "system";
            var msg = this.makemsg(type, content, type);
            this.showNewMsg(msg);
        };
        this.showFlymsg = function (msg) {
            if (msg) {
                var ggRoll = {
                    roll: '',
                    interval: "",
                    ...
                    limity: 0,
                    Go: function () {
                        this.roll.css('left', this.x + 'px');
                        this.roll.css('top', this.y + 'px');
                        /*
                    ...
                    start: function ($html) {
                        var self = this;
                        var roll_elemtent = $("<div class='fly_roll'></div>");
                        roll_elemtent.html($html);
                        roll_elemtent.appendTo("body");

                        self.roll = roll_elemtent;
                        self.x = $(window).width() + 100;
                        self.y = ($(window).height() - self.roll.height()) / 2;
                        var randomy = Math.random() * (80 - (-80)) + (-80);
                        self.y += randomy;

                        self.limitx = 0 - self.roll.width();
                        self.winW = $(window).width() - self.roll.width();
                        self.winH = $(window).height() - self.roll.height();

                        self.interval = setInterval(function () {
                            self.Go();
                        }, self.speed);
                        self.roll.hover(function () {
